...
describe('loginPage', () => {
  it('returns full HTML document', () => {
    const html = loginPage({ csrfToken: 'token' })
    expect(html).toContain('<!DOCTYPE html>')
    expect(html).toContain('</html>')
  })

  it('includes CSRF hidden input', () => {
    const html = loginPage({ csrfToken: 'my-csrf-token' })
  ...
  it('renders error message when provided', () => {
    const html = loginPage({ csrfToken: 'token', error: 'Invalid credentials' })
  ...
  it('escapes HTML in error message', () => {
    const html = loginPage({ csrfToken: 'token', error: '<script>xss</script>' })
